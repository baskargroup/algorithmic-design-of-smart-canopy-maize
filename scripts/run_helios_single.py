...
CATALOG_PATH_DEFAULT = "catalog/sim_catalog.parquet"
...
def run_helios(
    ...
    try:
        ...
        # Build command: optionally load modules, then run executable
        module_cmd = ""
        if use_modules and modules:
            module_cmd = " && ".join([f"module load {m}" for m in modules]) + " && "

        # Use bash -lc so `module` works (module is often a shell function)
...
def _ensure_catalog_exists(
    ...
    # Import here to keep run-time dependencies localized
    from generate_catalog import main as generate_catalog_main

    generate_catalog_main(n_sims=n_sims, out_path=catalog_path, lon_deg_fixed=lon_deg)
...
def run_helios_single(
    ...
    - Ensures canopy OBJ exists (generates if missing, when base_json_path is given)
    - Reads lat/lon from catalog
    - Copies base template -> per-sim INI
    - Runs Helios on specified GPU
    """
...
def main_cli():
    ...
    p.add_argument("--ensure_catalog", action="store_true",
                   help="If set, create catalog if missing.")
    p.add_argument("--n_sims", type=int, default=1,
                   help="Used only with --ensure_catalog (catalog size).")
    p.add_argument("--lon_deg", type=float, default=93.62,
                   help="Used only with --ensure_catalog (fixed lon).")
    ...
    p.add_argument("--ini_template", type=str, required=True,
                   help="Base Helios INI template to copy per sim (e.g., helios_config_gpu0.ini).")
